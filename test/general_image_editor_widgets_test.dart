...
Future<void> _selectEditorPanel(WidgetTester tester, String label) async {
  ...
  await tester.pump();
  await tester.tap(
    find.ancestor(of: labelFinder, matching: find.byType(ChoiceChip)).last,
  );
...
void main() {
  testWidgets('can select and delete an annotation from the visual preview', (
    ...
    await tester.ensureVisible(find.text('效果'));
    await tester.pump();
    ...
    await tester.ensureVisible(find.text('选区'));
    await tester.pump();
    await tester.tap(find.text('选区'));
    await _pumpBoundedSettle(tester);
